...
## Без своего сервера — P2P через Tailscale

Если не хочешь держать relay на VPS, поднимите канал прямо между двумя компами
через [Tailscale](https://tailscale.com) (бесплатно; он пробивает NAT, как
торрент-клиенты, и строит прямое зашифрованное соединение).

1. **Оба**: установите Tailscale и войдите в **один** аккаунт.
2. **Хост** (любой из двоих) узнаёт свой Tailscale-адрес:
   ```sh
   tailscale ip -4      # → 100.x.y.z
   ```
   и запускает relay+клиент одной командой:
   ```sh
   sync host 9000
   ```
3. **Второй** подключается к адресу хоста:
   ```sh
   sync client 100.x.y.z 9000
   ```

Готово — пробел ходит напрямую между вами, без VPS. (`sync host` сам поднимает
relay в фоне и подключает локального клиента; на macOS хосту тоже нужно
разрешение Accessibility, т.к. он ещё и участник.)

## Режимы

```
sync host <port>                # relay + клиент в одном процессе (P2P/Tailscale)
sync relay <port>               # только relay: пересылает байты между двумя клиентами
...
Один байт на событие:
- `0x00` — heartbeat (держит соединение живым, принимающий игнорит);
- `0x01` — пробел (play/pause);
- `0x02` — ← (перемотка назад);
- `0x03` — → (перемотка вперёд).

Без шифрования, без автообнаружения — IP и порт задаются явно. Перехват и
эмуляция этих клавиш синхронны на обоих компах: нажал у себя — нажалось у
второго. Если у вас один и тот же файл/видео и старт примерно вместе — play/pause
и перемотки держат вас в синхроне (без чтения времени плеера).
